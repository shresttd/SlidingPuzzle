...
public class SimplePuzzleState implements PuzzleState {
	...
	public PuzzleState shuffleBoard(int pathLength) {
		...
		for (i = 0; i < pathLength; i++){
			...
			rightRow = current.findEmptyRow();
			rightColumn = current.findEmptyColumn() + 1;
				
			leftRow = current.findEmptyRow();
			leftColumn = current.findEmptyColumn() - 1;
				
			topRow = current.findEmptyRow() - 1;
			topColumn = current.findEmptyColumn();
				
			bottomRow = current.findEmptyRow() + 1;
			bottomColumn = current.findEmptyRow(); 
			
			int lotto = current.lottery();
